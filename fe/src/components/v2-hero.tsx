    ...
    <header className="top">
      <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
        ...
        <span className="eyebrow" style={{ borderLeft: '1px solid var(--rule)', paddingLeft: 18 }}>
          Mantle Testnet · ERC-8004
        </span>
      </div>

      <nav style={{ display: 'flex', gap: 4 }}>
        {navItems.map(t => (
          <button key={t.id} className="btn btn-sm" onClick={() => setPage(t.id)}
            style={{
              padding: '8px 14px',
              color: page === t.id ? 'var(--paper)' : 'var(--paper-3)',
              fontWeight: 500,
              borderBottom: page === t.id ? '1px solid var(--accent)' : '1px solid transparent',
              borderRadius: 0,
            }}>
            {t.label}
          </button>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {wallet ? (
          <button className="btn btn-outline" onClick={onDisconnect}>
            <span style={{ width: 14, height: 14, borderRadius: '50%', background: 'linear-gradient(135deg, var(--accent), oklch(0.7 0.16 280))' }} />
            <span className="mono">{wallet.short}</span>
          </button>
        ) : (
  ...
  return (
    <section className="hero">
      <div>
        ...
        <p style={{ fontSize: 16, color: 'var(--paper-2)', lineHeight: 1.65, maxWidth: 580, marginTop: 24, textWrap: 'pretty' as React.CSSProperties['textWrap'] }}>
          {memo.body}
        </p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginTop: 24, color: 'var(--paper-3)', fontSize: 13 }}>
          ...
          <a href="#" className="mono" style={{ color: 'var(--paper-3)', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 4 }}>
            tx {memo.tx} <Icon name="external" size={11} />
          </a>
          <span style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--paper-4)' }} />
          <span className="mono dim">block #2,841,109</span>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: 24 }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: 18, marginTop: 14, fontSize: 12, color: 'var(--paper-3)' }}>
            <span><span className="eyebrow">24h</span> <span className="num pos" style={{ marginLeft: 6, fontSize: 13 }}>+{change24.toFixed(2)}%</span></span>
            <span><span className="eyebrow">YTD</span> <span className="num pos" style={{ marginLeft: 6, fontSize: 13 }}>+{ytd.toFixed(2)}%</span></span>
    ...
    <div style={{ flex: 1, position: 'relative', height: 14, display: 'flex', alignItems: 'center' }}>
      <svg width="100%" height="14" preserveAspectRatio="none" viewBox="0 0 100 14">
        {Array.from({ length: 21 }, (_, i) => {
          const x = (i / 20) * 100;
          const tall = i % 5 === 0;
          const filled = (i / 20) * 100 <= score;
          return (
            <line key={i} x1={x} x2={x} y1={tall ? 0 : 4} y2={tall ? 14 : 10}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          textAlign: 'center', fontSize: 9, color: 'var(--paper-3)',
